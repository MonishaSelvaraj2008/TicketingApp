...
namespace Assignment.Core.Handlers.Queries
{
    ...
    public class GetUserStoryByUserIdQueryHandler : IRequestHandler<GetUserStoryByUserIdQuery, IEnumerable<UserStoryDTO>>
    {
        ...
        public async Task<IEnumerable<UserStoryDTO>> Handle(GetUserStoryByUserIdQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                userStory = userStory.Where(obj=>obj.ResponsibleName.ToString().ToLower().Contains(request.Search)||
                                                obj.Status.ToString().ToLower().Contains(request.Search)||
                                                obj.Description.ToString().ToLower().Contains(request.Search)||
                                                obj.StoryPoint.ToString().ToLower().Contains(request.Search)||
                                                obj.AcceptanceCriteria.ToString().ToLower().Contains(request.Search)||
                                                obj.Comments.ToString().ToLower().Contains(request.Search));
